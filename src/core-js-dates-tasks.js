...
function getNextFriday(date) {
  ...
  if (getDay >= 5) {
    delta = 12 - date.getDay();
  }
  const fullMs = Date.parse(date) + delta * ms;
...
function isDateInPeriod(date, period) {
  ...
  const startDateMs = Date.parse(period.start);
  const endDateMs = Date.parse(period.end);
...
function getWeekNumberByDate(date) {
  ...
  const daysCurrentMonth = new Date(date).getDate();
  const count =
    Math.ceil((daysCount + daysCurrentMonth - daysFirstWeek) / 7) + 1;
  return count;
}
...
function isLeapYear(date) {
  ...
  return !(targetYear % 4 && targetYear % 400);
}
